...
<div class="flex items-center justify-center border">
    <form action="/register/step1" method="post" class="flex flex-col gap-5 w-200 place-content-center p-20" id="step1-form">
        <label>First Name
            <input type="text" value="<?= htmlspecialchars($steps['step1']['data']['first_name'] ?? '') ?>"
                   required name="first_name" class="border rounded-md w-70">
            <p class="hidden text-red-700" id="first_name_error"></p>
        </label>
        <label>Last Name
            <input type="text" value="<?= htmlspecialchars($steps['step1']['data']['last_name'] ?? '') ?>"
                   name="last_name"
                   required maxlength="100" class="border rounded-md w-70">
            <p class="hidden text-red-700" id="last_name_error"></p>
        </label>
        <label>Birthdate
            <input type="date" value="<?= htmlspecialchars($steps['step1']['data']['birthdate'] ?? '') ?>"
                   name="birthdate"
                   required class="border rounded-md w-70">
            <p class="hidden text-red-700" id="birthdate_error"></p>
        </label>
        <label>Report subject
            <input type="text" value="<?= htmlspecialchars($steps['step1']['data']['report_subject'] ?? '') ?>"
        ...
        <label>Phone
            <input type="tel" value="<?= htmlspecialchars($steps['step1']['data']['phone'] ?? '') ?>" name="phone"
                   required
                   maxlength="17"
        ...
        <label>Country
            <select name="country" class="border rounded-md w-70">
                <option value="<?= htmlspecialchars($steps['step1']['data']['country'] ?? '') ?>" selected>Choose
                    country
                </option>
        ...
        <label>Email
            <input type="email" value="<?= htmlspecialchars($steps['step1']['data']['email'] ?? '') ?>" name="email"
                   required class="border rounded-md w-70">
